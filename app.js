...
async function sGet(k){try{const r=await window.storage.get(k);return r?r.value:null;}catch(e){return null;}}
async function sSet(k,v){try{await window.storage.set(k,v);return true;}catch(e){storageOK=false;return false;}}
async function sDel(k){try{await window.storage.delete(k);}catch(e){}}

let saveTimer=null;
function scheduleSave(){
  const ind=$('#saveInd');ind.textContent='sichern…';ind.classList.add('show');
  clearTimeout(saveTimer);
  saveTimer=setTimeout(async()=>{
    await sSet(STATE_KEY,JSON.stringify(state));
...
function countDated(){let n=0;walk(state.tasks,t=>{if(t.start)n++;});return n;}
...
async function init(){
  ...
  if(!storageOK)toast('Speicher nicht verfügbar – nur diese Sitzung');
...
function appendTaskTiles(task,depth,inner){
  ...
  if(task.children.length && openMap[task.id]){
...
function attachDateDrag(handle,srcEl,task,fromTray){
  ...
  const updateIndicator=clientY=>{
    ...
    if(nestEl){
      mode='nest';nestId=nestEl.dataset.id;
      clearNest();nestEl.classList.add('nest-target');
  ...
  const up=e=>{
    ...
    scheduleSave();renderAll();toast((fromTray?'Terminiert auf ':'Verschoben auf ')+fmtLong(nd));
  };
...
function renderTree(){
  ...
  add.onclick=()=>{const nt=newTask('arbeit');state.tasks.push(nt);scheduleSave();renderAll();openSheet(nt.id);};
...
function renderNode(t,siblings,depth,container){
  ...
  if(hasKids&&open){
    ...
    add.onclick=()=>{const nt=newTask(t.cat==='termin'?'todo':t.cat);t.children.push(nt);openMap[t.id]=true;scheduleSave();renderAll();openSheet(nt.id);};
...
function confirmDelete(id){const f=findTask(id);if(!f)return;if(f.task.children.length&&!confirm('„'+(f.task.title||'Aufgabe')+'" inkl. '+f.task.children.length+' Unteraufgaben löschen?'))return;walk([f.task],t=>t.files.forEach(fl=>sDel(FILE_PREFIX+fl.id)));const i=f.list.findIndex(x=>x.id===id);f.list.splice(i,1);scheduleSave();closeSheet();renderAll();toast('Gelöscht');}

/* ===== detail sheet ===== */
let sheetId=null;
function openSheet(id){
  sheetId=id;const f=findTask(id);if(!f)return;const t=f.task;const path=pathTo(id)||[t];const c=CATS[t.cat].color;
  $('#sheet').innerHTML=`
...
function bindSheet(t){
  ...
  $('#f-title').oninput=e=>save('title',e.target.value);
  $('#f-title').onblur=renderAll;
  ...
  $('#f-start').onchange=e=>save('start',e.target.value);
  $('#f-end').onchange=e=>save('end',e.target.value);
  $$('#seg-cat .pill').forEach(b=>b.onclick=()=>{t.cat=b.dataset.v;scheduleSave();openSheet(t.id);});
  $$('#seg-prio .pill').forEach(b=>b.onclick=()=>{t.prio=b.dataset.v;scheduleSave();$$('#seg-prio .pill').forEach(x=>{x.classList.remove('on');x.style.background='';});b.classList.add('on');b.style.background=PRIOS[b.dataset.v].color;});
  $$('.crumbs a').forEach(a=>a.onclick=()=>openSheet(a.dataset.go));
  $('#doneToggle').onclick=()=>{toggleDone(t.id);openSheet(t.id);};
  const db=$('#detachBtn');if(db)db.onclick=()=>{detachTask(t.id);scheduleSave();renderAll();openSheet(t.id);toast('Als Hauptaufgabe gelöst');};
...
function handleFiles(list,t){
  [...list].forEach(file=>{
    ...
    reader.onload=async()=>{const id=uid();const data=reader.result;const ok=await sSet(FILE_PREFIX+id,data);fileCache[id]=data;t.files.push({id,name:file.name,type:file.type,size:file.size});scheduleSave();renderFiles(t);if(!ok)toast('Datei nur in dieser Sitzung gespeichert');};
...
/* ===== nav / tabs / action sheet ===== */
$('#tabbar').addEventListener('click',e=>{
  const b=e.target.closest('.tab');if(!b)return;currentView=b.dataset.view;
  $$('.tab').forEach(x=>x.classList.toggle('on',x===b));
  $$('.view').forEach(v=>v.classList.toggle('active',v.id==='view-'+currentView));
  $('#heuteBtn').style.display=currentView==='timeline'?'':'none';
  renderAll();
  if(currentView==='timeline')setTimeout(scrollToToday,60);
});
...
as.addEventListener('click',e=>{const b=e.target.closest('.as-item');if(!b)return;const cat=b.dataset.cat;const nt=newTask(cat);if(cat==='termin')nt.start=isoD(today());state.tasks.push(nt);scheduleSave();closeAS();renderAll();openSheet(nt.id);});
